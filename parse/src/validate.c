...
static line_token* validate_centre_aligned(validate_context* ctx, line_token* token)
{
	ctx->element_count += 3;
	...
	if (token->type != line_token_type_newline)
		handle_validate_error(ctx, "Centre-aligned text must be followed by a blank line.");
...
static void validate(line_tokens* tokens, doc_mem_req* out_mem_req)
{
	...
	for (;;)
	{
		...
		case line_token_type_centre_aligned:
			token = validate_centre_aligned(&ctx, token);
			break;
		case line_token_type_block_newline:
			handle_validate_error(&ctx, "Block quotes may not begin with a blank line.");
			break;
